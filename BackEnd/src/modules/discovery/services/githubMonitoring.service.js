...
import { getGitHubDiscoveryQueueStats } from "../workers/githubDiscovery.worker.js";
...
class GitHubMonitoringService {
  ...
  async getIngestionMetrics(timeRange = '24h') {
    const cacheKey = `ingestion-metrics-${timeRange}`;
    ...
    try {
      ...
      console.error('Error getting ingestion metrics:', error);
      throw error;
    }
  }

  async getProfileMetrics(timeRange = '24h') {
    const cacheKey = `profile-metrics-${timeRange}`;
    ...
    try {
      // Get GitHub candidates created in time range
      const candidates = await GitHubCandidateMetadata.find({
      ...
      const metrics = {
        ...
        profiles: {
          ...
          avgDeveloperScore: this.calculateAvgDeveloperScore(candidates),
          avgContributionScore: this.calculateAvgContributionScore(candidates)
        },
        seniority: this.getSeniorityDistribution(candidates),
        specialization: this.getSpecializationDistribution(candidates),
        skills: this.getSkillDistribution(candidates),
        quality: this.getQualityMetrics(candidates),
        activity: this.getActivityMetrics(candidates)
      };
  ...
  async getRealTimeMetrics() {
    try {
      ...
      const activeCandidates = await GitHubCandidateMetadata.getActiveDevelopers(1, 10);
  ...
  async getHealthStatus() {
    try {
      ...
      if (queueStats.failed > 10) {
        ...
        health.recommendations.push('Check GitHub API rate limits and authentication');
      }
      ...
      if (recentFailures.length > 5) {
        ...
        health.recommendations.push('Review error logs and adjust retry strategy');
      }
  ...
  calculateAvgDeveloperScore(candidates) {
    if (candidates.length === 0) return 0;
    const totalScore = candidates.reduce((sum, c) => sum + (c.developerScore || 0), 0);
    return (totalScore / candidates.length).toFixed(2);
  }

  calculateAvgContributionScore(candidates) {
    if (candidates.length === 0) return 0;
    const totalScore = candidates.reduce((sum, c) => sum + (c.contributionScore || 0), 0);
    return (totalScore / candidates.length).toFixed(2);
  }
  ...
  getSeniorityDistribution(candidates) {
    ...
    candidates.forEach(candidate => {
      const seniority = candidate.inferredSkills?.seniority || 'unknown';
  ...
  getSpecializationDistribution(candidates) {
    const distribution = {};
    candidates.forEach(candidate => {
      const specializations = candidate.inferredSkills?.specialization || [];
  ...
  getSkillDistribution(candidates) {
    const distribution = {};
    candidates.forEach(candidate => {
      const skills = candidate.inferredSkills?.all || [];
  ...
  getQualityMetrics(candidates) {
    ...
    candidates.forEach(candidate => {
      ...
      metrics.avgDataFreshness += quality.dataFreshness || 0;
      metrics.avgActivityScore += quality.activityScore || 0;
      metrics.avgCredibilityScore += quality.credibilityScore || 0;
    });
  ...
  getActivityMetrics(candidates) {
    const active = candidates.filter(c => c.isActiveDeveloper(6)).length;
    const veryActive = candidates.filter(c => c.isActiveDeveloper(1)).length;
    
    return {
      total: candidates.length,
      active6Months: active,
      active1Month: veryActive,
      activeRate: ((active / candidates.length) * 100).toFixed(2)
    };
  }
  ...
  calculateAvgProfilesPerSecond(history) {
    ...
    const totalProfiles = completed.reduce((sum, h) => sum + (h.result?.stats?.totalSaved || 0), 0);
    const totalTime = completed.reduce((sum, h) => sum + h.duration, 0);
  ...
  calculateDuplicateRate(history) {
    ...
    const totalDuplicates = completed.reduce((sum, h) => {
      return sum + (h.result?.stats?.totalDeduplicated || 0);
    }, 0);
    
    const totalProfiles = completed.reduce((sum, h) => {
      return sum + (h.result?.stats?.totalFetched || 0);
    }, 0);
  ...
  generateRecommendations(ingestionMetrics, profileMetrics, performanceMetrics, healthStatus) {
    ...
    if (ingestionMetrics.queue.waiting > 50) {
      recommendations.push({
        ...
        action: 'Increase GitHub discovery worker concurrency'
      });
    }

    // Quality recommendations
    if (profileMetrics.profiles.avgDeveloperScore < 30) {
      recommendations.push({
        type: 'quality',
        priority: 'medium',
        message: 'Average developer score is low. Consider refining search criteria.',
        action: 'Adjust GitHub search queries to target higher quality candidates'
      });
    }

    // Performance recommendations
    if (performanceMetrics.performance.avgProcessingTime > 30000) {
      recommendations.push({
        type: 'performance',
        priority: 'medium',
        message: 'Processing times are high. Consider optimizing profile processing.',
        action: 'Review and optimize GitHub profile parsing and skill inference'
      });
    }
...
export const githubMonitoringService = new GitHubMonitoringService();
export default githubMonitoringService;

...
export const register = async (req, res) => {
  try {
    ...
    let userExists =
      ...
      (await DigitalMarketer.findOne({ "emailId.email": email }));
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Your Digital Account Has Been Created",
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                  <div style="text-align: center; margin-bottom: 20px;">
                    <h2>Great<span style="color: #1D4ED8;">Hire</span></h2>
                    <p style="color: #555;">Building Smart and Powerful Digital Marketer Teams</p>
                  </div>
            
                  <h3 style="color: #333;">Welcome to Great<span style="color: #1D4ED8;">Hire</span>, ${fullname}!</h3>
                  <p style="color: #555;">
                    We are excited to inform you that you have been added as a Digital Marketer GreatHire. Below are your account details:
                  </p>
...
//login section for Digital Marketer...
export const login = async (req, res) => {
  try {
    ...
    let user = await DigitalMarketer.findOne({ "emailId.email": email });
    ...
    const userWithoutPassword = await DigitalMarketer.findById(user._id).select(
      "-password"
    );

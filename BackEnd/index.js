...
server.listen(PORT, async () => {
  ...
  const isReplicaSet = () => {
      ...
      return topology && topology.description.type === "ReplicaSetWithPrimary";
    };
  // Change stream handlers
  const setupChangeStream = (model, errorMsg) => {
  if (!isReplicaSet()) {
    console.warn(`⚠️ Skipping ChangeStream for ${model.modelName} (Not a replica set)`);
    return;
  }
  ...
  try {
    ...
    console.error(`${errorMsg} (init failed):`, err.message);
    return;
  }
 ...
 if (isReplicaSet()) {
  ...
  notificationChangeStream.on("error", (err) => {
    console.error("Notification ChangeStream error:", err.message);
  });
} else {
  console.warn("⚠️ Notification ChangeStream skipped (Not a replica set)");
}
});

...
const handleApply = async (jobId) => {
  ...
  // Ye check ab bilkul sahi kaam karega kyunki state update ho rahi hai
  if (hasAppliedToJob(jobId)) {
    toast.error("Already applied");
    return;
  }
  ...
  try {
    ...
    const payload = {
      ...
      applicantProfile: user.profile,   // poora profile object
      resume: user?.profile?.resume,    // resume link bhi bhej do
    };
    const response = await axios.post(
      `${JOB_API_END_POINT}/${jobId}/apply`,
      payload,
    ...
    if (response.data?.success) {
      const newApplication = { applicant: user._id };
      updateJobInList(jobId, newApplication);
    ...
    const errorMessage = err.response?.data?.message || "Error while applying";

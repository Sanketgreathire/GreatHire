        ...
        <div className="w-11/12 max-w-4xl bg-white shadow-lg rounded-2xl p-6">
          ...
          {user?.role === "recruiter" && app.status === "Pending" ? (
            <div className="mt-6 flex justify-end gap-4">
              <button
                className={`px-6 py-2 bg-green-500 text-white rounded-lg shadow hover:bg-green-600 ${(loading === 1 || loading === -1) && "cursor-not-allowed"
                  }`}
              ...
              <button
                className={`px-6 py-2 bg-red-500 text-white rounded-lg shadow hover:bg-red-600 ${(loading === 1 || loading === -1) && "cursor-not-allowed"
                  }`}
                disabled={loading === 1 || loading === -1}
                onClick={() => updateStatus(-1)}
              >
            ...
            <p
              className={`flex justify-end ${app.status === "Shortlisted" ? "text-green-600" : "text-red-600"
                }`}
            >
              {app.status}
            </p>

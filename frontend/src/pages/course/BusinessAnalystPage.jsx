...
const TOOLS = [
  { name: "MS Visio", color: "bg-blue-100 text-blue-700", icon: "📐" },
  { name: "JIRA", color: "bg-indigo-100 text-indigo-700", icon: "🎯" },
  { name: "Confluence", color: "bg-blue-100 text-blue-800", icon: "📝" },
  { name: "Balsamiq", color: "bg-orange-100 text-orange-700", icon: "🖼️" },
  { name: "Lucidchart", color: "bg-yellow-100 text-yellow-700", icon: "🔷" },
  { name: "Power BI", color: "bg-amber-100 text-amber-700", icon: "📊" },
  { name: "Excel", color: "bg-green-100 text-green-700", icon: "📋" },
  { name: "SQL", color: "bg-gray-100 text-gray-700", icon: "🗄️" },
  { name: "Figma", color: "bg-pink-100 text-pink-700", icon: "🎨" },
  { name: "Trello", color: "bg-cyan-100 text-cyan-700", icon: "📌" },
  { name: "Miro", color: "bg-yellow-100 text-yellow-800", icon: "🟡" },
  { name: "MS Office", color: "bg-red-100 text-red-700", icon: "📄" },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative">
        ...
        {done ? (
          ...
          <div className="p-6">
            <div className="mb-5 pb-4 border-b border-gray-100"><p className="text-xs text-indigo-600 font-bold uppercase tracking-widest mb-1">Book Free Demo</p><h3 className="text-xl font-black text-gray-900">Business Analytics</h3><p className="text-sm text-gray-500 mt-1">🎯 Free demo class — no commitment required!</p></div>
            <div className="space-y-4">
              {[{ label: "Full Name", key: "name", type: "text", placeholder: "Your full name" }, { label: "Email Address", key: "email", type: "email", placeholder: "you@example.com" }, { label: "Phone Number", key: "phone", type: "tel", placeholder: "+91 98765 43210" }].map(({ label, key, type, placeholder }) => (
                <div key={key}><label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1.5">{label}</label><input required type={type} placeholder={placeholder} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
              ))}
              <div><label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1.5">Preferred Mode</label><select value={form.mode} onChange={(e) => setForm({ ...form, mode: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><option>Online</option><option>Offline</option><option>Hybrid</option></select></div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/v1/courses/enquiry`, {
        ...
        body: JSON.stringify({ ...form, courseName: "Business Analytics", fee: "₹25,000", type: "enrollment" }),
      });
    } catch (_) {}
    setLoading(false);
    setDone(true);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative">
        ...
        {done ? (
          <div className="p-8 text-center">
            ...
            <button onClick={onClose} className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-semibold text-sm hover:bg-indigo-700">Got it!</button>
          </div>
        ) : (
          <div className="p-6">
            <div className="mb-5 pb-4 border-b border-gray-100">
              <p className="text-xs text-indigo-600 font-bold uppercase tracking-widest mb-1">Enroll Now</p>
              <h3 className="text-xl font-black text-gray-900">Business Analyst Course</h3>
              <p className="text-sm text-gray-500 mt-1">⚡ Course Fee: <span className="font-bold text-indigo-600">₹25,000</span> · EMI from ₹7,000/mo</p>
            </div>
            <div className="space-y-4">
              {[{ label: "Full Name", key: "name", type: "text", placeholder: "Your full name" }, { label: "Email Address", key: "email", type: "email", placeholder: "you@example.com" }, { label: "Phone Number", key: "phone", type: "tel", placeholder: "+91 98765 43210" }].map(({ label, key, type, placeholder }) => (
                <div key={key}>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1.5">{label}</label>
                  <input required type={type} placeholder={placeholder} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              ))}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-1.5">Batch</label>
                  <select value={form.batch} onChange={(e) => setForm({ ...form, batch: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
export default function BusinessAnalystPage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="hidden lg:block">
              <div className="bg-white rounded-2xl shadow-2xl p-6 text-gray-900">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700">4.8</span><span className="text-xs text-gray-400">(1,600+ reviews)</span></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-14">
            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-4 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Course Overview</h2>
              <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 mb-6">
                <p className="text-gray-700 text-base leading-relaxed mb-4">The <strong>Business Analyst Course</strong> at Great Hire is a 3.5-month job-oriented program covering the full BA lifecycle — from requirements elicitation and BPMN process modelling to Agile-Scrum, SQL, and stakeholder reporting with Power BI.</p>
                <p className="text-gray-700 text-base leading-relaxed">With real BRD and SRS projects, live Agile sprint simulations, and direct placement support through Great Hire's 500+ company network, this is the most practical BA course in Hyderabad.</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[{ icon: "⏱", label: "Duration", val: "3.5 Months" }, { icon: "📚", label: "Modules", val: "7 Modules" }, { icon: "🖥", label: "Mode", val: "Online + Offline" }, { icon: "🌐", label: "Language", val: "English / Telugu" }].map((item) => (
                  <div key={item.label} className="bg-white border border-gray-200 rounded-xl p-4 text-center shadow-sm"><p className="text-2xl mb-1">{item.icon}</p><p className="text-xs text-gray-400 font-medium mb-0.5">{item.label}</p><p className="text-sm font-bold text-gray-900">{item.val}</p></div>
                ))}
              </div>
            </section>

            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-6 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Why This Course?</h2>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl sm:text-3xl font-black text-gray-900 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Course Curriculum</h2>
            ...
            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-6 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Tools & Technologies</h2>
            ...
            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-6 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Upcoming Batches</h2>
              <div className="space-y-4">
                {BATCHES.map((b) => (
            ...
            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-6 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Student Reviews</h2>
            ...
            <section>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 mb-6 flex items-center gap-2"><span className="w-1 h-7 bg-indigo-600 rounded-full inline-block"></span>Frequently Asked Questions</h2>
          ...
          <div className="hidden lg:block">
            <div className="sticky top-24 space-y-5">
              <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-6">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700">4.8</span><span className="text-xs text-gray-400">(1,600+)</span></div>
      ...
      <section className="bg-gradient-to-r from-indigo-700 to-violet-700 py-14 mt-10">

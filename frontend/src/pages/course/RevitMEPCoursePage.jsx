...
const TOOLS = [
  { name: "Revit MEP 2025", color: "bg-blue-100 text-blue-700", icon: "🏗️" },
  { name: "Revit Architecture", color: "bg-indigo-100 text-indigo-700", icon: "🏛️" },
  { name: "Navisworks", color: "bg-purple-100 text-purple-700", icon: "🔍" },
  { name: "AutoCAD MEP", color: "bg-orange-100 text-orange-700", icon: "📐" },
  { name: "BIM 360", color: "bg-teal-100 text-teal-700", icon: "☁️" },
  { name: "Dynamo", color: "bg-yellow-100 text-yellow-700", icon: "⚡" },
  { name: "Insight 360", color: "bg-green-100 text-green-700", icon: "💡" },
  { name: "IFC Viewer", color: "bg-pink-100 text-pink-700", icon: "📦" },
  { name: "Hevacomp", color: "bg-red-100 text-red-700", icon: "🌡️" },
  { name: "SketchUp Pro", color: "bg-emerald-100 text-emerald-700", icon: "🧊" },
];
    ...
    <div className={`border rounded-xl overflow-hidden transition-all duration-200 ${isOpen ? "border-teal-300 shadow-sm" : "border-gray-200"}`}>
      <button onClick={onToggle} className="w-full flex items-center justify-between px-5 py-4 text-left bg-white hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-3 min-w-0">
          <span className="shrink-0 text-xs font-bold bg-teal-100 text-teal-700 px-2.5 py-1 rounded-full">{item.module}</span>
      ...
      {isOpen && (
        <div className="px-5 pb-5 bg-teal-50/40 border-t border-teal-100">
          <p className="text-xs text-teal-700 font-semibold mb-3 mt-3 uppercase tracking-wider">Topics Covered</p>
        ...
        {done ? (
          ...
          <div className="p-6">
            <div className="mb-5 pb-4 border-b border-gray-100">
              ...
              <h3 className="text-xl font-black text-gray-900">Revit MEP Course</h3>
            ...
            <div className="space-y-4">
              {[{ label: "Full Name", key: "name", type: "text", placeholder: "Your full name" }, { label: "Email Address", key: "email", type: "email", placeholder: "you@example.com" }, { label: "Phone Number", key: "phone", type: "tel", placeholder: "+91 98765 43210" }].map(({ label, key, type, placeholder }) => (
                <div key={key}>
                  ...
                  <input required type={type} placeholder={placeholder} value={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500" />
                </div>
              ))}
              ...
              <button onClick={() => form.name && form.email && form.phone && setDone(true)} className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3.5 rounded-xl text-sm transition-colors mt-1">Book Free Demo Class →</button>
...
export default function RevitMEPCoursePage() {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900">
      <Navbar/>
      <section className="bg-gradient-to-br from-slate-900 via-teal-950 to-cyan-900 text-white relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="hidden lg:block">
              <div className="bg-white rounded-2xl shadow-2xl p-6 text-gray-900">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700">4.9</span><span className="text-xs text-gray-400">(2,600+ reviews)</span></div>
                ...
                <button onClick={() => setShowModal(true)} className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3.5 rounded-xl text-sm transition-colors mb-3">Book Free Demo Class</button>
      ...
      <div className="lg:hidden bg-white border-b border-gray-200 sticky top-16 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          ...
          <button onClick={() => setShowModal(true)} className="bg-teal-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm whitespace-nowrap">Enroll Now</button>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-14">
            <section>
              ...
              <div className="bg-teal-50 border border-teal-100 rounded-2xl p-6 mb-6">
                <p className="text-gray-700 text-base leading-relaxed mb-4">The <strong>Revit MEP Course</strong> at Great Hire is a 3-month BIM-focused program covering complete MEP (Mechanical, Electrical & Plumbing) design using Autodesk Revit — the most in-demand tool in the global construction and infrastructure industry.</p>
                <p className="text-gray-700 text-base leading-relaxed">From HVAC duct routing to electrical panel schedules and plumbing drainage systems, you'll design complete building services for a real project, preparing you for immediate deployment at any MEP firm.</p>
              </div>
            ...
            <section>
              ...
              <div className="mt-4 text-center"><button className="text-teal-600 text-sm font-semibold hover:underline">📥 Download Complete Syllabus PDF</button></div>
            </section>
                ...
                {BATCHES.map((b) => (
                  <div key={b.type} className={`flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-2xl border gap-4 ${b.urgent ? "border-teal-300 bg-teal-50" : "border-gray-200 bg-white"}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 ${b.urgent ? "bg-teal-100" : "bg-gray-100"}`}>📅</div>
                      <div>
                    ...
                    <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                      ...
                      <button onClick={() => setShowModal(true)} className="bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs px-4 py-2 rounded-lg whitespace-nowrap transition-colors">Enroll →</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="hidden lg:block">
            <div className="sticky top-24 space-y-5">
              <div className="bg-white border border-gray-200 rounded-2xl shadow-lg p-6">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700">4.9</span><span className="text-xs text-gray-400">(2,600+)</span></div>
                ...
                <button onClick={() => setShowModal(true)} className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3.5 rounded-xl text-sm mb-3 transition-colors">Book Free Demo Class</button>
              ...
              <div className="bg-gradient-to-br from-teal-600 to-cyan-700 rounded-2xl p-5 text-white text-center">
                ...
                <button className="bg-white text-teal-700 font-bold text-xs px-4 py-2 rounded-lg hover:bg-teal-50 transition-colors">Share & Earn</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-gradient-to-r from-teal-700 to-cyan-700 py-14 mt-10">

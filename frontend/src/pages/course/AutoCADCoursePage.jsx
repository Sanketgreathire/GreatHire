...
const TOOLS = [
  { name: "AutoCAD 2025", color: "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300", icon: "📐" },
  { name: "AutoCAD LT", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300", icon: "✏️" },
  { name: "AutoCAD Civil 3D", color: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300", icon: "🏗️" },
  { name: "AutoCAD Architecture", color: "bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300", icon: "🏛️" },
  { name: "Autodesk Inventor", color: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300", icon: "⚙️" },
  { name: "Navisworks", color: "bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300", icon: "🔍" },
  { name: "Revit Basics", color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300", icon: "🏠" },
  { name: "SketchUp", color: "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300", icon: "📦" },
  { name: "PDF Underlay", color: "bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300", icon: "📄" },
  { name: "BricsCAD", color: "bg-pink-100 text-pink-700 dark:bg-pink-900/40 dark:text-pink-300", icon: "🖊️" },
];
    ...
    <div className={`border rounded-xl overflow-hidden transition-all duration-200 ${isOpen ? "border-orange-300 dark:border-orange-600 shadow-sm" : "border-gray-200 dark:border-gray-700"}`}>
      <button onClick={onToggle} className="w-full flex items-center justify-between px-5 py-4 text-left bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/60 transition-colors">
        <div className="flex items-center gap-3 min-w-0">
          <span className="shrink-0 text-xs font-bold bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300 px-2.5 py-1 rounded-full">{item.module}</span>
        ...
        <div className="flex items-center gap-3 shrink-0 ml-2">
          ...
          <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all duration-200 ${isOpen ? "bg-orange-600 text-white rotate-180" : "bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300"}`}>
      ...
      {isOpen && (
        <div className="px-5 pb-5 bg-orange-50/40 dark:bg-gray-800/80 border-t border-orange-100 dark:border-gray-700">
          <p className="text-xs text-orange-700 dark:text-orange-400 font-semibold mb-3 mt-3 uppercase tracking-wider">Topics Covered</p>
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/v1/courses/enquiry`, {
        ...
        body: JSON.stringify({ ...form, courseName: "AutoCAD", type: "demo" }),
      });
      setLoading(false);
      setDone(true);
    } catch (_) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md relative border border-gray-100 dark:border-gray-700">
        ...
        {done ? (
          ...
          <div className="p-6">
            <div className="mb-5 pb-4 border-b border-gray-100 dark:border-gray-700"><p className="text-xs text-orange-600 dark:text-orange-400 font-bold uppercase tracking-widest mb-1">Book Free Demo</p><h3 className="text-xl font-black text-gray-900 dark:text-white">AutoCAD</h3><p className="text-sm text-gray-500 dark:text-gray-400 mt-1">🎯 Free demo class — no commitment required!</p></div>
            <div className="space-y-4">
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white ${
              ...
              <div><label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide block mb-1.5">Preferred Mode</label><select value={form.mode} onChange={(e) => setForm({ ...form, mode: e.target.value })} className="w-full border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"><option className="dark:bg-gray-800">Online</option><option className="dark:bg-gray-800">Offline</option><option className="dark:bg-gray-800">Hybrid</option></select></div>
...
export default function AutoCADCoursePage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 font-sans text-gray-900 dark:text-gray-100">
      ...
      {/* Hero Banner */}
      <section className="bg-gradient-to-br from-slate-900 via-orange-950 to-red-900 dark:from-gray-950 dark:via-orange-950 dark:to-slate-900 text-white relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="hidden lg:block">
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 text-gray-900 dark:text-white border border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700 dark:text-gray-200">4.8</span><span className="text-xs text-gray-400">(4,100+ reviews)</span></div>
                <p className="text-3xl font-black text-orange-600 dark:text-orange-400 mb-1">₹38,000</p>
                ...
                <button onClick={() => setShowDemo(true)} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3.5 rounded-xl text-sm transition-colors mb-3">
      ...
      <div className="lg:hidden bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-16 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div><p className="text-xl font-black text-orange-600 dark:text-orange-400 leading-none">₹38,000</p><p className="text-xs text-gray-400 dark:text-gray-400">EMI from ₹7,000/mo</p></div>
          <button onClick={() => setShowEnroll(true)} className="bg-orange-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm whitespace-nowrap">Enroll Now</button>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-14">
            ...
            <section>
              ...
              <div className="bg-orange-50 dark:bg-gray-800/60 border border-orange-100 dark:border-gray-700 rounded-2xl p-6 mb-6">
                <p className="text-gray-700 dark:text-gray-300 text-base leading-relaxed mb-4">The <strong className="text-gray-900 dark:text-white">AutoCAD Course</strong> at Great Hire is a comprehensive 3-month job-oriented program covering 2D drafting, 3D modeling, and domain-specific applications for civil, mechanical, and architectural engineering.</p>
                <p className="text-gray-700 dark:text-gray-300 text-base leading-relaxed">With industry-standard drawing projects, Autodesk certification guidance, and direct placement support through Great Hire's 500+ company network, this course prepares you for a high-demand career in design and drafting.</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {HIGHLIGHTS.map((h) => (
                  <div key={h.title} className="flex gap-4 p-5 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl hover:border-orange-200 dark:hover:border-orange-600 hover:shadow-sm transition-all group">
                    <div className="w-11 h-11 bg-orange-50 dark:bg-gray-700 group-hover:bg-orange-100 dark:group-hover:bg-gray-600 rounded-xl flex items-center justify-center text-xl shrink-0 transition-colors">{h.icon}</div>
            ...
            <section>
              ...
              <div className="mt-4 text-center"><button className="text-orange-600 dark:text-orange-400 text-sm font-semibold hover:underline">📥 Download Complete Syllabus PDF</button></div>
            </section>
                ...
                {BATCHES.map((b) => (
                  <div key={b.type} className={`flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-2xl border gap-4 ${b.urgent ? "border-orange-300 dark:border-orange-600 bg-orange-50/50 dark:bg-gray-800/80" : "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"}`}>
                    ...
                    <div className="flex items-center gap-3 sm:flex-col sm:items-end">
                      ...
                      <button onClick={() => setShowEnroll(true)} className="bg-orange-600 hover:bg-orange-700 text-white font-semibold text-xs px-4 py-2 rounded-lg whitespace-nowrap transition-colors">Enroll →</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="hidden lg:block">
            <div className="sticky top-24 space-y-5">
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-lg p-6">
                <div className="flex items-center gap-2 mb-1"><Stars /><span className="text-sm font-bold text-gray-700 dark:text-gray-200">4.8</span><span className="text-xs text-gray-400">(4,100+)</span></div>
                <p className="text-3xl font-black text-orange-600 dark:text-orange-400 mb-1">₹38,000</p>
                ...
                <button onClick={() => setShowDemo(true)} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3.5 rounded-xl text-sm mb-3 transition-colors">
                  Book Free Demo Class
                </button>
              ...
              <div className="bg-gradient-to-br from-orange-600 to-red-700 dark:from-orange-700 dark:to-red-800 rounded-2xl p-5 text-white text-center">
                ...
                <a
                  href={`https://wa.me/?text=${encodeURIComponent("📐 Check out this AutoCAD Course at Great Hire! Master 2D & 3D design with 100% placement support.\n" + window.location.href)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-white text-orange-600 font-bold text-xs px-4 py-2 rounded-lg hover:bg-orange-50 transition-colors">
                  Share & Earn
                </a>
              </div>
            </div>
      ...
      {/* Footer CTA Banner */}
      <section className="bg-gradient-to-r from-orange-700 to-red-700 dark:from-orange-800 dark:to-red-900 py-14 mt-10">
      ...
      {showEnroll && <CourseEnrollModal onClose={() => setShowEnroll(false)} courseName="AutoCAD" amount={38000} accentColor="red" />}
      <Suspense fallback={null}>
        {showCounsellor && <TalkToCounsellorModal courseName="AutoCAD" onClose={() => setShowCounsellor(false)} />}
      </Suspense>
    </div>
  );
}
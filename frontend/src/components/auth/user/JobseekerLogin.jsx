...
const JobseekerLogin = () => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      const response = showOtpInput
        ? await axios.post(`${USER_API_END_POINT}/verify-jobseeker-otp`, { email: formData.email, otp }, { withCredentials: true })
        : await axios.post(`${USER_API_END_POINT}/jobseeker-login`, formData, { withCredentials: true });
      ...
      if (response?.data?.success) {
        ...
        if (rememberMe) {
          ...
          localStorage.setItem("rememberedJobseekerPassword", showOtpInput ? otp : formData.password);
        } else {
        ...
        if (user.role === "student" || user.role === "candidate") {
          navigate(user.isFirstLogin ? "/profile" : (from || "/"));
  ...
  }, [formData, otp, showOtpInput, rememberMe, dispatch, navigate, location]);
            ...
            <div className="relative w-full h-full flex items-center justify-center">
              <div className="relative w-full h-full overflow-hidden rounded-2xl">
                <div className="absolute inset-0 flex transition-transform duration-1000 ease-in-out" style={slideStyle}>
                  {SLIDES.map((slide, index) => (
                    <div key={index} className="min-w-full relative">
                      <img src={slide.image} alt={slide.title} className="w-full h-full object-cover" loading={index === 0 ? "eager" : "lazy"} decoding="async" />
                ...
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 flex space-x-2">
                  {SLIDES.map((_, index) => (
                    <button key={index} onClick={() => setCurrentSlide(index)} className={`h-2 rounded-full transition-all duration-300 ${currentSlide === index ? "bg-white w-8" : "bg-white/50 w-2"}`} />
                  ))}
                </div>
              </div>
            ...
            <div className="w-full max-w-md space-y-6">
              <div className="text-center">
                ...
                <p className="mt-4 text-sm text-gray-700 dark:text-gray-300">
                  ...
                  <span onClick={handleSignUpClick} className="text-blue-600 dark:text-blue-400 cursor-pointer underline font-semibold hover:text-blue-800 transition-colors">
                    Sign Up
                  </span>
                </p>
              </div>
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Enter your email"
                      className="block w-full pl-10 pr-4 py-3 text-sm border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                ...
                {!showOtpInput ? (
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Enter your password"
                        ...
                        style={showPassword ? {webkitTextSecurity:'none', fontFamily:'inherit'} : {}}
                  ...
                  <div>
                    ...
                    <input type="text" name="otp" value={otp} onChange={handleOtpChange} placeholder="Enter 6-digit OTP"
                      className="block w-full px-4 py-3 text-sm border-2 border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl text-center text-lg tracking-wider focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    ...
                    <div className="mt-3 text-center">
                      {resendTimer > 0
                        ...
                        : <button type="button" onClick={handleSendOtp} disabled={loading} className="text-blue-600 dark:text-blue-400 text-sm font-semibold hover:underline disabled:opacity-50">{otpSent ? "Resend OTP" : "Send OTP"}</button>
                      }
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <input id="remember-me" type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="h-4 w-4 text-blue-600 border-gray-300 rounded" />
                    <label htmlFor="remember-me" className="ml-2 text-sm text-gray-700 dark:text-gray-300 font-medium">Remember me</label>
                  </div>
                  <button type="button" onClick={handleForgotPassword} className="text-blue-600 dark:text-blue-400 text-sm hover:underline font-semibold">Forgot Password?</button>
                </div>

                {/* Submit */}
                <button type="submit" disabled={loading}
                  className={`w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold py-4 rounded-xl text-sm transition-all duration-200 ${loading ? "opacity-70 cursor-not-allowed" : "hover:from-blue-700 hover:to-purple-700 hover:shadow-xl"}`}>
                  {loading
